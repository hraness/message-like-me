...
export const LOCAL_MESSAGE_BUNDLE_V2_SCHEMA_VERSION = MESSAGE_BUNDLE_V2_SCHEMA_IDENTITY;
export const LOCAL_MESSAGE_BUNDLE_V2_FORMAT = "message-like-me.local-message-bundle" as const;
export const LOCAL_MESSAGE_BUNDLE_V2_SOURCE_ID = "wacli-local" as const;
export const LOCAL_MESSAGE_BUNDLE_V2_SOURCE_TRANSFORM_VERSION = "1.0.0" as const;
export const LOCAL_MESSAGE_BUNDLE_V2_PROVIDER_ID = "whatsapp" as const;
export const LOCAL_MESSAGE_BUNDLE_V2_PROVIDER_VERSION = "0.15.0" as const;
export const LOCAL_MESSAGE_BUNDLE_V2_NETWORK = "whatsapp" as const;
 ...
 */
export const LOCAL_MESSAGE_BUNDLE_V2_SUPPORTED_SOURCE_TRANSFORM_VERSIONS = Object.freeze([
  LOCAL_MESSAGE_BUNDLE_V2_SOURCE_TRANSFORM_VERSION,
] as const);

export const LOCAL_MESSAGE_BUNDLE_V2_LIMITS = Object.freeze({
...
export const LOCAL_MESSAGE_BUNDLE_V2_ARTIFACTS = Object.freeze([
...
export type LocalMessageBundleV2RecordKind =
  typeof LOCAL_MESSAGE_BUNDLE_V2_ARTIFACTS[number]["kind"];

export type LocalMessageBundleV2ArtifactPath =
  typeof LOCAL_MESSAGE_BUNDLE_V2_ARTIFACTS[number]["path"];
...
export type LocalMessageBundleV2Record =
...
export class MessageBundleV2ContractError extends TypeError {
  readonly code = "message-bundle-v2-contract" as const;
...
export function parseLocalMessageBundleV2WhatsAppJid(
  value: unknown,
  label = "WhatsApp JID",
): LocalMessageBundleV2WhatsAppJid {
...
function exactJidHandle(value: unknown, jid: LocalMessageBundleV2WhatsAppJid, label: string): string | null {
...
export function isLocalMessageBundleV2SourceTransformVersion(
  value: unknown,
): value is typeof LOCAL_MESSAGE_BUNDLE_V2_SOURCE_TRANSFORM_VERSION {
  return value === LOCAL_MESSAGE_BUNDLE_V2_SOURCE_TRANSFORM_VERSION;
}

export function assertLocalMessageBundleV2SourceTransformVersion(
  value: unknown,
  label = "manifest.source.version",
): typeof LOCAL_MESSAGE_BUNDLE_V2_SOURCE_TRANSFORM_VERSION {
  const parsed = version(value, label);
  if (!isLocalMessageBundleV2SourceTransformVersion(parsed)) {
    return fail(
      `${label} must be a supported source transform: ${LOCAL_MESSAGE_BUNDLE_V2_SUPPORTED_SOURCE_TRANSFORM_VERSIONS.join(", ")}`,
    );
  }
  return parsed;
}

function parseProvenance(value: unknown, label: string): LocalMessageBundleV2Provenance {
...
function parseCommon(
  ...
  label: string,
): Omit<LocalMessageBundleV2CommonRecord, "kind"> & Readonly<{
  kind: LocalMessageBundleV2RecordKind;
}> {
  exactKeys(record, [
  ...
  const connectedAccount = parseLocalMessageBundleV2WhatsAppJid(
    object(record.provenance, `${label}.provenance`).connectedAccountProviderId,
    `${label}.provenance.connectedAccountProviderId`,
  );
...
function parseAccount(
  ...
  const providerJid = parseLocalMessageBundleV2WhatsAppJid(
    common.provenance.providerId,
    `${label}.provenance.providerId`,
  );
  ...
  return Object.freeze({
    ...
    displayName: nullableText(
      record.displayName,
      `${label}.displayName`,
      LOCAL_MESSAGE_BUNDLE_V2_LIMITS.shortTextBytes,
    ),
...
function parseConversation(
  ...
  if (
    (type === "direct" && providerJid.kind === "group")
    || (type === "group" && providerJid.kind !== "group")
  ) return fail(`${label} type conflicts with its WhatsApp JID`);
  const participantIds = identifiers(
    record.participantIds,
    `${label}.participantIds`,
    LOCAL_MESSAGE_BUNDLE_V2_LIMITS.participantsPerConversation,
  );
  const participantsComplete = nullableBoolean(
...
function parseReply(value: unknown, label: string): LocalMessageBundleV2Reply | null {
...
function parseDeletion(value: unknown, label: string): LocalMessageBundleV2Deletion | null {
  if (value === null) return null;
  const record = object(value, label);
...
function parseAttachments(
  value: unknown,
  label: string,
): readonly LocalMessageBundleV2AttachmentRecord[] {
  return Object.freeze(array(
    value,
    label,
    LOCAL_MESSAGE_BUNDLE_V2_LIMITS.attachmentsPerMessage,
  ).map((item, index) => {
    ...
    return Object.freeze({
      ...
      mimeType: nullableText(
        record.mimeType,
        `${itemLabel}.mimeType`,
        LOCAL_MESSAGE_BUNDLE_V2_LIMITS.mimeTypeBytes,
      ),
      name,
...
function parseMessage(
  ...
  const body = nullableText(
    record.body,
    `${label}.body`,
    LOCAL_MESSAGE_BUNDLE_V2_LIMITS.bodyBytes,
  );
...
export function parseLocalMessageBundleV2Record<Kind extends LocalMessageBundleV2RecordKind>(
  value: unknown,
  kind: Kind,
  label = `${kind} record`,
): LocalMessageBundleV2RecordByKind[Kind] {
  const record = object(value, label);
  switch (kind) {
    case "account": return parseAccount(record, label) as LocalMessageBundleV2RecordByKind[Kind];
    case "participant": return parseParticipant(record, label) as LocalMessageBundleV2RecordByKind[Kind];
    case "conversation": return parseConversation(record, label) as LocalMessageBundleV2RecordByKind[Kind];
    case "message": return parseMessage(record, label) as LocalMessageBundleV2RecordByKind[Kind];
    case "reaction": return parseReaction(record, label) as LocalMessageBundleV2RecordByKind[Kind];
    case "tombstone": return parseTombstone(record, label) as LocalMessageBundleV2RecordByKind[Kind];
  }
}

function parseArtifact(value: unknown, index: number): LocalMessageBundleV2Artifact {
...
export function localMessageBundleV2ManifestProjection(
  manifest: LocalMessageBundleV2Manifest,
): LocalMessageBundleV2ManifestProjection {
  const { integrity: _integrity, ...projection } = manifest;
  return Object.freeze(projection);
}

export function localMessageBundleV2BundleSha256(
  projection: LocalMessageBundleV2ManifestProjection,
): string {
  return sha256(canonicalJson(projection));
}

export function parseLocalMessageBundleV2Manifest(value: unknown): LocalMessageBundleV2Manifest {
  ...
  if (
    ...
    || record.format !== LOCAL_MESSAGE_BUNDLE_V2_FORMAT
  ...
  const warnings = array(
    record.warnings,
    "manifest.warnings",
    LOCAL_MESSAGE_BUNDLE_V2_LIMITS.warnings,
  ...
  exactKeys(
    counts,
    LOCAL_MESSAGE_BUNDLE_V2_ARTIFACTS.map(({ kind }) => kind),
    "manifest.counts",
  );
  ...
  ])) as Record<LocalMessageBundleV2RecordKind, number>;
  if (parsedCounts.account > LOCAL_MESSAGE_BUNDLE_V2_LIMITS.accounts) {
    return fail(
      `Manifest exceeds the ${LOCAL_MESSAGE_BUNDLE_V2_LIMITS.accounts}-account safety bound`,
    );
  }
  ...
  const artifactValues = array(
    record.artifacts,
    "manifest.artifacts",
    LOCAL_MESSAGE_BUNDLE_V2_ARTIFACTS.length,
  );
  ...
  if (
    totalRecords > LOCAL_MESSAGE_BUNDLE_V2_LIMITS.records
    || totalBytes > LOCAL_MESSAGE_BUNDLE_V2_LIMITS.totalBytes
  ...
  const result: LocalMessageBundleV2Manifest = Object.freeze({
    schemaVersion: LOCAL_MESSAGE_BUNDLE_V2_SCHEMA_VERSION,
    ...
    provider: Object.freeze({
      ...
      version: (() => {
        ...
        if (parsed !== LOCAL_MESSAGE_BUNDLE_V2_PROVIDER_VERSION) {
          return fail(
            `manifest.provider.version must be ${LOCAL_MESSAGE_BUNDLE_V2_PROVIDER_VERSION}`,
          );
        }
  ...
  if (
    localMessageBundleV2BundleSha256(localMessageBundleV2ManifestProjection(result))
