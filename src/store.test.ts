...
describe("local corpus store", () => {
  ...
  test("reactivates a duplicate reaction when the preferred reaction target is suppressed", async () => {
    const root = await mkdtemp(join(tmpdir(), "message-like-me-x-reaction-target-suppression-"));
    const store = LocalStore.open(join(root, "store.sqlite3"));
    try {
      store.replaceSources([xBeeperSnapshot()], "2026-08-21T00:01:00.000Z");
      store.replaceSources(
      ...
      store.replaceSources([{
        ...
        source: {
          ...
          generatedAt: "2026-08-27T03:02:10.221Z",
          manifestSha256: "e".repeat(64),
        },
        deletions: [{
          entityKind: "message",
          localEntityId: "x-archive-message-overlap",
          externalId: "x-archive-external-overlap",
          deletedAt: "2026-08-27T03:02:10.221Z",
          expectedConversationId: X_ARCHIVE_CONVERSATION_ID,
          reason: "tombstone",
        }],
