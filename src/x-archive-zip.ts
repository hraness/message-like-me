...
const MAX_TOTAL_SELECTED_BYTES = 768 * 1024 * 1024;
const MAX_TOTAL_DECLARED_BYTES = 64 * 1024 * 1024 * 1024;
const MAX_NAME_BYTES = 4 * 1024;
const MAX_RATIO = 200;

export type ExtractedXArchiveMember = Readonly<{
  memberName: string;
  logicalName: string;
  bytes: Uint8Array;
}>;

export type XArchiveMembers = Readonly<{
  manifest: ExtractedXArchiveMember;
  account: ExtractedXArchiveMember;
  directMessages: ExtractedXArchiveMember | null;
  groupDirectMessages: ExtractedXArchiveMember | null;
  directMessageHeaders: ExtractedXArchiveMember | null;
  groupDirectMessageHeaders: ExtractedXArchiveMember | null;
  identityMetadata: readonly ExtractedXArchiveMember[];
}>;
...
const SELECTED = /^(?:[^/]+\/)?data\/(?:manifest|account|direct-message(?:-group)?-headers|direct-messages(?:-group)?[^/]*|tweets|deleted-tweets|community-tweet)\.js$/u;
...
function directoryFromEocd(descriptor: number, archiveSize: number): Directory {
  ...
  for (let offset = tail.length - 22; offset >= 0; offset -= 1) {
    if (
      tail.readUInt32LE(offset) === EOCD
      && tailOffset + offset + 22 + tail.readUInt16LE(offset + 20) === archiveSize
    ) candidates.push(tailOffset + offset);
  }
  if (candidates.length !== 1) {
    throw new Error(candidates.length === 0
      ? "X ZIP end-of-central-directory record is missing"
      : "X ZIP end-of-central-directory record is ambiguous");
  }
  ...
  const hasZip64 = locatorOffset >= 0
    && readExact(descriptor, locatorOffset, 4, "ZIP64 locator signature").readUInt32LE(0) === ZIP64_LOCATOR;
  if (!hasZip64) {
    if (
      [legacyDisk, legacyCentralDisk, legacyOnDisk, legacyCount].includes(U16_MAX)
      || [legacySize, legacyOffset].includes(U32_MAX)
  ...
  if (
    zip64Offset + zip64.length !== locatorOffset
...
function validateType(entry: Entry): void {
  ...
  const unixType = host === UNIX_HOST || host === MACOS_HOST
    ? (entry.externalAttributes >>> 16) & UNIX_TYPE_MASK
    : 0;
  if (entry.directory) {
    if (
      entry.crc32 !== 0
      || entry.uncompressedSize !== 0
      || (entry.method === STORED && entry.compressedSize !== 0)
    ...
    (entry.externalAttributes & DOS_DIRECTORY) !== 0
    || (unixType !== 0 && unixType !== UNIX_REGULAR)
...
function centralEntries(descriptor: number, directory: Directory, archiveSize: number): Entry[] {
  ...
  for (let index = 0; index < directory.count; index += 1) {
    ...
    let zip64Position = 0;
    const nextZip64 = (label: string): number => {
      if (zip64 === null) throw new Error(`X ZIP ${label} is missing ZIP64 metadata`);
      const value = u64(zip64, zip64Position, label);
      zip64Position += 8;
      return value;
    };
    const uncompressedSize = uncompressedLegacy === U32_MAX
      ? nextZip64("uncompressed size")
      : uncompressedLegacy;
    const compressedSize = compressedLegacy === U32_MAX
      ? nextZip64("compressed size")
      : compressedLegacy;
    const localHeaderOffset = offsetLegacy === U32_MAX
      ? nextZip64("local header offset")
      : offsetLegacy;
    let disk = diskLegacy;
    ...
    if (![compressedSize, uncompressedSize, localHeaderOffset].every(
    ...
    if (
      ...
      && (
        ...
        || uncompressedSize > MAX_X_ZIP_MEMBER_BYTES
      )
...
function localSizes(
  ...
  label: string,
): { compressed: number; uncompressed: number; zip64: boolean } {
  const zip64 = fields.get(ZIP64_EXTRA) ?? null;
...
function readSelected(descriptor: number, entry: Entry, range: LocalRange): ExtractedXArchiveMember {
  const compressed = readExact(
    descriptor,
    range.dataOffset,
    range.dataEnd - range.dataOffset,
    `selected member ${entry.name}`,
  );
  let output: Buffer;
  try {
    output = entry.method === STORED
  ...
  if (((updateCrc32(0xffffffff, output) ^ 0xffffffff) >>> 0) !== entry.crc32) {
...
/** Validate an official X ZIP completely and extract only bounded messaging evidence. */
export function extractXArchiveFile(descriptor: number, archiveSize: number): XArchiveMembers {
