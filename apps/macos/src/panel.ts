...
const capabilityNames = { messages: "Messages", contacts: "Contacts", agent: "Agent account", attachments: "File attachments", reactions: "Reactions", stickers: "Stickers", "mini-apps": "iMessage apps" };
...
export function mountPanel(root: HTMLElement, port: DesktopControlPort): void {
  ...
  function setup(): string {
    return `<section class="section"><h2>Connection & capabilities</h2><p class="section-description">Textbutler uses Ghostget for Messages and Contacts. Each feature becomes available only after the connected provider confirms it.</p><div class="capability-list">${snapshot.capabilities.map(capability => `<div class="capability-row"><div><strong>${capabilityNames[capability.id]}</strong><p>${escape(capability.detail)}</p></div><span class="capability-state ${capability.status}">${statusNames[capability.status]}</span></div>`).join("")}</div></section>`;
  }
  function behavior(): string {
    ...
    return `<form id="behavior-form"><section class="section"><div class="setting-row activation-row"><div><h2>Butler for this contact</h2><p>Allow the butler to respond in this conversation.</p></div><label class="switch"><input type="checkbox" name="enabled" ${draft.enabled ? "checked" : ""} aria-label="Enable butler for this contact" ${!editable() ? "disabled" : ""}><span></span></label></div></section>
      ...
      <div class="setting-row"><label for="provider"><strong>Agent provider</strong><span>Uses your connected coding-agent account.</span></label><select id="provider" name="provider" ${!editable() ? "disabled" : ""}><option value="codex" ${draft.provider === "codex" ? "selected" : ""}>Codex</option><option value="claude" ${draft.provider === "claude" ? "selected" : ""}>Claude Code</option></select></div><p class="quiet-note">The daemon selects a low-cost classifier for this provider. Provider setup is shown in the Setup tab.</p></section>
