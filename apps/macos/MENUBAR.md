...
The Textbutler desktop distribution includes a native macOS status-item companion
beside its bundled CLI. Launch and login installation are explicit owner actions.
...
This directory contains the Swift source and the explicit build/run helper. The
development build produces an unbundled binary. Desktop packaging inventories
it as a native resource, signs it separately with the selected Developer ID,
and notarizes it inside the app. The published desktop coordinate must advance
before distributing new bytes. `menubar install` copies the admitted binary to
a private per-user location and installs its separate login service.
...
Run the bundled CLI using its `textbutler-bun` runtime and `cli.ts` entry. It installs
the release-built binary for login startup with
`textbutler menubar install`. Launching never builds source. The companion uses
...
fixture sockets under `/private/tmp`, including malformed responses, size
