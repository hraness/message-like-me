...
function exactToken(value) {
  if (
    ...
    fail("MLM_RELEASE_APP_TOKEN is missing or malformed");
  }
  return value;
}
...
export function advanceWebsiteProductionRef(options) {
  ...
  const token = exactToken(options.environment.MLM_RELEASE_APP_TOKEN);
  const arguments_ = websiteProductionPushArguments(options.expectedOldSha, options.verifiedSha);
  const temporaryDirectory = mkdtempSync(join(tmpdir(), "message-like-me-release-askpass-"));
