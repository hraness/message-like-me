...
async function readBoundedJson(response, label) {
  ...
  if (bytes.byteLength > MAX_RESPONSE_BYTES) fail(`${label} exceeded its response bound`);
  try {
...
async function revokeWithFetch(input) {
  const endpoint = new URL("/installation/token", input.apiUrl);
  const response = await fetch(endpoint, {
    headers: {
      Accept: "application/vnd.github+json",
  ...
  if (response.status !== 204) {
    fail(`release App token revocation returned HTTP ${String(response.status)}`);
  }
  const bytes = new Uint8Array(await response.arrayBuffer());
