...
export default function AboutPage() {
  return <><SiteHeader /><main className="document-page" id="main-content" tabIndex={-1}>
    ...
    <section className="about-sources" aria-labelledby="about-build-title"><div><h2 id="about-build-title">Building from clear boundaries</h2></div><div><p>The Mac app and local daemon foundations are in source. Ghostget will own Messages and Contacts permissions and transport. Agentrouter is the reusable foundation for selected Codex and Claude accounts and scoped execution. Live automatic replies remain disabled until those integrations are qualified. There is no signed Mac download yet.</p><a href={ARCHITECTURE_URL}>Read the architecture</a></div></section>
    <section className="about-sources" aria-labelledby="about-history-title"><div><h2 id="about-history-title">What we are carrying forward</h2></div><div><p>Textbutler grew out of the unused Message Like Me experiment. Its history readers and evidence methodology remain available as legacy tools. They inform contact memory, but their read-only import contracts do not establish live messaging support for the new app.</p><Link href="/sources">Explore the legacy history tools</Link></div></section>
