...
export default function AboutPage() {
  return <><SiteHeader /><main className="document-page" id="main-content" tabIndex={-1}>
    ...
    <section className="about-sources" aria-labelledby="about-build-title"><div><h2 id="about-build-title">Building from clear boundaries</h2></div><div><p>The Mac app and daemon are in source, with iMessage and WhatsApp connections through Ghostget. Replies require explicit messaging permissions, a ready Claude API account, and contact activation. Claude Code and Codex remain unavailable while their execution boundaries are qualified. New installations start paused, live delivery needs account-specific verification, and there is no signed Mac download yet.</p><a href={ARCHITECTURE_URL}>Read the architecture</a></div></section>
