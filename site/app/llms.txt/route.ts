...
export function GET() {
  const body = [
    ...
    'Textbutler is a macOS app and daemon for a personal message butler. The source includes iMessage and WhatsApp setup through Ghostget, direct-conversation enrollment, optional recent-history import, editable memory, hooks, smart and keyword-only response controls, global pause, and an active contact limit (default five). New installations start paused and new contacts start disabled.',
    ...
    'Replies require a configured Ghostget connection, a ready explicit agent account, an enabled contact, and global resume. Claude API is the current separately billed option in the packaged runtime; native Claude Code and Codex remain unavailable. There is no signed Textbutler Mac download yet. Live delivery still needs verification on the selected account.',
