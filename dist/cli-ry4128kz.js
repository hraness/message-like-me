...
function assertLocalMessageBundleV1SourceTransformVersion(value, label = "manifest.source.version") {
  ...
  if (!isLocalMessageBundleV1SourceTransformVersion(parsed)) {
    return fail(`${label} must be a supported source transform: ${LOCAL_MESSAGE_BUNDLE_V1_SUPPORTED_SOURCE_TRANSFORM_VERSIONS.join(", ")}`);
  }
  return parsed;
}
...
function parseAttachments(value, label) {
  return Object.freeze(array(value, label, LOCAL_MESSAGE_BUNDLE_V1_LIMITS.attachmentsPerMessage).map((item, index) => {
...
function parseReaction(record, label) {
  ...
  return Object.freeze({
    ...
    body: boundedText(record.body, `${label}.body`, LOCAL_MESSAGE_BUNDLE_V1_LIMITS.shortTextBytes),
...
function parseLocalMessageBundleV1Manifest(value) {
  ...
  const warnings = array(record.warnings, "manifest.warnings", LOCAL_MESSAGE_BUNDLE_V1_LIMITS.warnings).map((item, index) => token(item, `manifest.warnings[${index}]`));
  ...
  exactKeys(counts, LOCAL_MESSAGE_BUNDLE_V1_ARTIFACTS.map(({ kind }) => kind), "manifest.counts");
  ...
  if (parsedCounts.account > LOCAL_MESSAGE_BUNDLE_V1_LIMITS.accounts) {
    return fail(`Manifest exceeds the ${LOCAL_MESSAGE_BUNDLE_V1_LIMITS.accounts}-account safety bound`);
  }
  const artifactValues = array(record.artifacts, "manifest.artifacts", LOCAL_MESSAGE_BUNDLE_V1_ARTIFACTS.length);
...
export { LOCAL_MESSAGE_BUNDLE_V1_SCHEMA_VERSION, LOCAL_MESSAGE_BUNDLE_V1_FORMAT, LOCAL_MESSAGE_BUNDLE_V1_SOURCE_ID, LOCAL_MESSAGE_BUNDLE_V1_SOURCE_TRANSFORM_VERSION, LOCAL_MESSAGE_BUNDLE_V1_PROVIDER_ID, LOCAL_MESSAGE_BUNDLE_V1_SUPPORTED_SOURCE_TRANSFORM_VERSIONS, LOCAL_MESSAGE_BUNDLE_V1_LIMITS, LOCAL_MESSAGE_BUNDLE_V1_ARTIFACTS, MessageBundleV1ContractError, isLocalMessageBundleV1SourceTransformVersion, assertLocalMessageBundleV1SourceTransformVersion, parseLocalMessageBundleV1Record, localMessageBundleV1ManifestProjection, localMessageBundleV1BundleSha256, parseLocalMessageBundleV1Manifest };
