...
- A native WhatsApp bundle is still only a finished offline input. Do not
  ...
  matching. The verified Wrench v0.16.3 producer omits reaction-shaped Wacli
  rows with `reaction-state-unproven`; never turn that missing evidence into a
  claim that no reactions occurred.
