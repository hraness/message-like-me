...
The currently verified Wrench v0.16.3/Wacli v0.15.0 producer omits every
reaction-shaped row and reports `reaction-state-unproven` when it encounters
one because current active or removed state is not durable. Treat that warning,
and an empty reaction artifact from this producer, as unobservable reaction
behavior. Do not infer that the user avoided reactions or compare a native
WhatsApp reaction ratio. The v2 wire contract can parse a proven reaction
record, but this exact producer does not supply one.
