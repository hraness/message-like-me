...
function fixture(options: { leases?: Leases; factoryFailure?: boolean; operationTimeoutMs?: number; closeTimeoutMs?: number } = {}) {
  ...
  let readGate: ReturnType<typeof deferred<void>> | undefined, closeGate: ReturnType<typeof deferred<void>> | undefined;
  ...
  const controller = createManagedCodexAccountController({ accountId: "account-one", owner: "owner-one", processGeneration: 3, leases,
  ...
  return { controller, leases, calls, requests, entered,
    ...
    challenge(value: unknown) { challengeOverride = value; }, logoutFailure() { logoutFailure = true; },
    holdRead() { return readGate = deferred<void>(); }, holdClose() { return closeGate = deferred<void>(); },
...
describe("managed Codex account controller", () => {
  ...
  test.each(["logout", "login", "cancel"] as const)("own %s account notification before RPC response cannot publish stale success", async method => {
    ...
    const operation = method === "logout" ? f.controller.logout() : method === "login" ? f.controller.startLogin("chatgpt") : f.controller.cancelLogin("login-one");
