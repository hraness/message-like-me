...
describe("Codex task adapter custody", () => {
  ...
  test("execution plus cleanup above one hour fails before launch with its own cleanup proof", async () => {
    const f = fixture(), task = f.task("account-one", 3_590_000, 20_000);
    try {
      // The generic task runtime refuses this total before acquiring a lease.
      // Call the adapter directly to prove its independent preflight boundary.
      const result = await f.adapter.run(task.execution, task.broker);
      ...
      const stopped = await f.adapter.stop(task.execution, "failed");
      expect(stopped.runId).toBe(task.request.runId); expect(stopped.accountId).toBe(task.request.accountId);
      expect(stopped.processStopped).toBe(true); expect(stopped.joined).toBe(true);
    } finally {
      await f.adapter.stop(task.execution, "cancelled").catch(() => undefined); await task.broker.close(); f.db.close();
    }
  });

  test("a thirty-second host cleanup allowance remains separate from execution time", async () => {
    const f = fixture(), task = f.task("account-one", 60_000, 30_000);
    try {
      const result = await task.run();
      expect(result.outcome.status).toBe("completed"); expect(result.output).toBe("Synthetic finding.");
      expect(result.timing.outerDeadlineUnixMs - result.timing.executionDeadlineUnixMs).toBe(30_000);
      expect(f.counts()).toMatchObject({ launchCalls: 1, stopCalls: 1, upstreamCalls: 1 });
      expect(f.leases.inspect("codex", task.request.accountId)).toBeNull();
    } finally { task.controller.abort(); f.releaseResponse(); await task.broker.close(); f.db.close(); }
  });
