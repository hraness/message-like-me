...
export function assertCodexProtocolManifest(value: unknown, runtime: Readonly<{
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("CODEX_PROTOCOL_MANIFEST_INVALID");
  const raw = value as Record<string, unknown>;
  const keys = ["protocol", "protocolVersion", "sourceVersion", "executableSha256", "schemaSha256", "manifestSha256", "generatedAtUnixMs"];
  if (Object.keys(raw).length !== keys.length || Object.keys(raw).some(key => !keys.includes(key))) throw new Error("CODEX_PROTOCOL_MANIFEST_INVALID");
