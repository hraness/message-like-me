...
/** Separate candidate admission: preserve v1 and add metadata for the /var
 * symlink used by the system resolver. This grants no file contents, additional
 * network destinations or services. DNS evidence does not qualify sign-in. */
...
export function createCodexAccountProcess(options: CodexAccountProcessOptions, trustedSystem: CodexAccountProcessSystem = system): CodexAccountOwnedProcessPort {
  ...
  const receipt = (): CodexAccountProcessReceipt => Object.freeze({ schema: "agentrouter.codex-account-process.v1", binding: owned, productionQualified: false, network, nativeVersion: runtime.version,
