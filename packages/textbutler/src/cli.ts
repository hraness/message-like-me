...
export const CLI_USAGE = "textbutler init|doctor|providers list|providers check ACCOUNT|daemon run|daemon install|daemon uninstall|daemon status|menubar|menubar install|menubar uninstall|menubar status [--data-dir /physical/private/path]";
export async function runTextbutlerCli(argv: readonly string[], output: { write(text: string): unknown } = process.stdout, options: { launchAgent?: LaunchAgentLifecycle;
  ...
  if (!["init", "doctor", "providers list", "daemon run", "daemon install", "daemon uninstall", "daemon status", "menubar", "menubar install", "menubar uninstall", "menubar status"].includes(command) && !checkAccount) throw new Error(CLI_USAGE);
  ...
  if (command === "menubar status") {
    const binary = options.menuBarBinary ?? installedMenuBarBinary();
    const lifecycle = options.menuBarLaunchAgent ?? createMenuBarLaunchAgentLifecycle(binary);
    const launchAgent = await lifecycle.status(dataDir);
    const ok = launchAgent.installation === "installed" || launchAgent.installation === "absent";
  ...
  if (command === "menubar install" || command === "menubar uninstall") {
    if (process.platform !== "darwin" && options.menuBarLaunchAgent === undefined) throw new Error("The Textbutler menu bar is supported on macOS only.");
  ...
  if (command === "menubar") {
    ...
    return await runMenuBar(binary, dataDir);
  }
  if (command === "doctor") {
    try {
